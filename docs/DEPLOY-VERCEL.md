...
| Processing     | Immediately after each delivery, in-process | The same, but the instance is frozen the moment the 200 is sent, so that work is handed to `waitUntil` too. Without that it sat half-finished until the next request thawed the instance — a scan could take a minute or more to reach the register. |
| Live register  | One process; reconnects replay what was missed | A stream lives 15 s (`maxDuration`), then the browser reconnects — possibly to another instance with its own event buffer. The server says so in its greeting (`continuity: "none"`), and the register fetches the screen again on every reconnect. In effect a 15-second poll: a scan appears within about 15 s of being processed, not instantly. |
